...
void Solaris2::Initialize()
{
	...
	ss_font = std::make_unique<dbf::Font>("Fonts/Lady Radical.ttf", 13);
	studioTitle = std::make_unique<dbf::Text>(ss_font.get());
	studioTitle->Create(dbf::g_renderer, "drachenblut games presents:", { 225, 225, 225, 225 });
...
void Solaris2::Draw(dbf::Renderer& renderer)
{
	...
	if (titleTimer > 0) {
		studioTitle->Draw(renderer, { 325,340 });
